...
void error(){
	printf("ERREUR: Le fichier dot ne sera pas generé car certaines variables/fonctions ne sont pas bien declaré/appelé.\n");
...
Value lookup(Env* env, char* symbol){
	for(int i = env->size-1; i >= 0; --i){
    		if(strcmp(env->liste_symbol[i],symbol)==0){
...
int dot_affectation (struct affectation_t affectation,Env env){
	...
	index_graph= index_graph+1;
		...
		dot_variable(affectation.variable,env);
		int index_expr = dot_expression(affectation.expression,env);
		fprintf(fichier,"node_%d -> node_%d\n", myIndex,index_var );
		fprintf(fichier,"node_%d -> node_%d\n", myIndex,index_expr );
...
void extend_env_liste_declaration (liste_declarations_t l,Env* env){
	for (int i = 0; i < l.size; ++i){
		for (int j = 0; j < l.liste[i].liste_declarateurs.size; ++j){
			extend_env(env, l.liste[i].liste_declarateurs.liste[j].identificateur,(Value){.type = 1, .exp.declarateur = l.liste[i].liste_declarateurs.liste[j]});
		}
	}
...
int dot_variable(variable_t t,Env env){
	...
	if(d.liste_constantes.size != t.liste_expressions->size){
		error();
	}
	...
	if(t.liste_expressions->size == 0){
		...
		for (int i = 0; i < t.liste_expressions->size; ++i){
			int index_list_expr = dot_expression(t.liste_expressions->liste[i],env);
